...
<ParamField path="get_end_of_response_token" type="bool">
  Specifies whether you wish to receive an end-of-response token. The end-of-response token is a '<END>' as of now. So the WebSocket will send this string as a response once all audio data is sent.
</ParamField>
